...
import {
  LayoutDashboard, Store, LayoutList, UtensilsCrossed,
  LogOut, ChevronRight, Menu, X, ShoppingBag,
  ClipboardList, ChevronLeft,
} from "lucide-react";

const navItems = [
  { href: "/owner", label: "Overview", icon: LayoutDashboard },
  { href: "/owner/store", label: "Toko Saya", icon: Store },
  { href: "/owner/categories", label: "Kategori", icon: LayoutList },
  { href: "/owner/menu-items", label: "Menu", icon: ShoppingBag },
  { href: "/owner/orders", label: "Pesanan", icon: ClipboardList },
];
...
export default function OwnerLayout({ children }: { children: React.ReactNode }) {
  ...
  const toggleCollapsed = () => {
    setCollapsed((prev) => {
      localStorage.setItem("owner_sidebar_collapsed", String(!prev));
      return !prev;
    });
  };
          ...
          {!collapsed && (
            <>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-amber-400 text-xs">Owner Panel</p>
              </div>
              <button
                onClick={() => setSidebarOpen(false)}
