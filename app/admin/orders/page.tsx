...
import {
  ...
  ShoppingBag, TrendingUp, X, Store,
} from "lucide-react";
import { storesApi, Store as StoreType } from "@/lib/storesApi";
import { adminOrdersApi, Order } from "@/lib/ordersApi";
...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [ordersData, summaryData] = await Promise.all([
        adminOrdersApi.getAll(selectedStore.id, activeFilter),
        adminOrdersApi.dailySummary(selectedStore.id),
      ]);
  ...
  const handleNext = async (id: string) => {
    setLoadingId(id);
    try {
      await adminOrdersApi.nextStatus(id);
      fetchOrders(true);
  ...
  const handleCancel = async (id: string) => {
    ...
    setLoadingId(id);
    try {
      await adminOrdersApi.setStatus(id, "CANCELLED");
      fetchOrders(true);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl sm:text-2xl font-black text-white">Semua Pesanan</h1>
            {pendingCount > 0 && (
          ...
          <p className="text-stone-400 mt-1 text-sm">Monitor pesanan dari semua toko</p>
        </div>
        <button
      ...
      {loadingStores ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-stone-400 text-sm">Belum ada toko yang terdaftar.</p>
        </div>
      ) : (
        <>
          {/* Pilih toko */}
          ...
          {loadingOrders ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="text-white font-bold text-lg mb-2">
                {activeFilter
                  ? `Tidak ada pesanan ${STATUS_CONFIG[activeFilter as keyof typeof STATUS_CONFIG]?.label}`
                  : "Belum ada pesanan"
                }
              </h3>
